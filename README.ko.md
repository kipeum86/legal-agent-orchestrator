...
이 리포에 실제로 있는 케이스 하나다. 모의 데이터가 아니다. 아래에서 언급하는 모든 파일은 [`output/20260410-012238-391f/`](output/20260410-012238-391f/) 아래에 그대로 있다.
...
실제 47-event 케이스 파일 ([`output/20260410-012238-391f/events.jsonl`](output/20260410-012238-391f/events.jsonl))에서 15건을 뽑아봤다:
...
위 [실제 케이스 한눈에 보기](#실제-케이스-한눈에-보기)와 같은 케이스를 이번엔 제대로 풀어본다. 언급되는 파일은 전부 [`output/20260410-012238-391f/`](output/20260410-012238-391f/) 아래 실재하므로 직접 `cat`해서 볼 수 있다.
...
[`research-result.md`](output/20260410-012238-391f/research-result.md) (37 KB) + [`research-meta.json`](output/20260410-012238-391f/research-meta.json) (10 KB) 참조.
...
리뷰 전 v1 초안은 [`opinion-v1.md`](output/20260410-012238-391f/opinion-v1.md) (39 KB)에서 확인할 수 있다.

### Stage 3 — 리뷰 (반성문, 파트너) — 잡아낸 것들

파트너가 `approved_with_revisions`를 리턴하면서 **9개 코멘트 — Critical 2건, Major 3건, Minor 4건** 달았다. 일반적인 스타일 지적이 아니다. 그중 두 건을 [`review-meta.json`](output/20260410-012238-391f/review-meta.json)에서 그대로 인용:
...
전체 코멘트 세트는 라인 레벨 참조까지 포함된 [`review-result.md`](output/20260410-012238-391f/review-result.md) (23 KB)에서 볼 수 있다.
...
오케스트레이터가 직접 수정본을 읽고, `korean-law` MCP로 §38, §31-2, §47 1차 소스 원문을 쿼리한 뒤 verbatim diff를 돌렸다. 전체 rescue 로그는 [`verbatim-verification.md`](output/20260410-012238-391f/verbatim-verification.md) (5 KB)에 있다.
...
| 항목 | 값 |
|------|-----|
| 케이스 | [`20260410-012238-391f`](output/20260410-012238-391f/) |
...
|------|-----------|------|------|-------------|------|-----------|------|
| **T1** | `PIPA-expert` 단독 | direct | 9 | 8 / 1 / 0 | ~60K | 582초 | [`test-T1-20260410-121640/`](output/test-T1-20260410-121640/). `library/grade-b/` KB gap 발견, 이후 해소. |
| **Regression** | `game-legal-research` 단독 | direct | 32 | 25 / 0 / 7 | ~170K | 797초 | [`test-regression-20260410-121640/`](output/test-regression-20260410-121640/). 전문가 vs v1 범용 baseline: −3% comparable, 11/11 주제 coverage. |
| **T2** | `[PIPA ∥ GDPR]` 병렬 | Pattern 1 | 26 | 26 / 0 / 0 | ~124K | 334초 | [`test-T2-20260410-121640/`](output/test-T2-20260410-121640/). 양 브랜치 실제 병렬 실행, 5-dimension 다관할권 태깅 정상. |
...
- 모든 파일 `verification_status: VERIFIED`, MCP 원문 verbatim 인용. [lowtidebuild/PIPA-expert@6b8137c](https://github.com/lowtidebuild/PIPA-expert/commit/6b8137c) 참조.
...
│   ├── 20260410-012238-391f/           # Phase 1 E2E 샘플 케이스
...
| [output/20260410-012238-391f/](output/20260410-012238-391f/) | 이 README 전반에 걸쳐 참조된 샘플 케이스 |
