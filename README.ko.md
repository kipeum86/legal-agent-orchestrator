...
```mermaid
flowchart LR
    O["오케스트레이터<br/>200K context<br/><i>실사용 ~25-40K</i>"]
    A["서브에이전트 A<br/><i>fresh 200K</i><br/>CLAUDE.md + skills + MCP"]
    B["서브에이전트 B<br/><i>fresh 200K</i><br/>CLAUDE.md + skills + MCP"]
    C["서브에이전트 C<br/><i>fresh 200K</i><br/>CLAUDE.md + skills + MCP"]
    O -->|Agent tool 호출| A
    O -->|Agent tool 호출| B
    O -->|Agent tool 호출| C
    A -.->|파일로 결과 반환| O
...
실패 모드까지 영구 기록에 남는다. [Phase 1 E2E 케이스](samples/20260410-012238-391f/events.jsonl)에서 리비전 도중 rate limit 에러(`evt_044`)가 터졌을 때, 오케스트레이터가 직접 `korean-law` MCP로 인용구 대조를 수행해서 리비전을 살려냈다(`evt_045`). 단일 LLM 시스템에서는 그냥 "모델 에러"로 끝났을 것이 여기서는 append-only 로그의 typed 이벤트로 남는다. **그게 "프로세스 자체가 프로덕트"의 진짜 의미다.**
...
flowchart TB
    ...
    subgraph Subs["서브에이전트 · 독립된 Claude 인스턴스, 각각 fresh 200K ctx"]
        direction LR
        SA["서브에이전트 A<br/>자체 CLAUDE.md<br/>자체 skills<br/>자체 KB<br/>자체 MCP 도구"]
        SB["서브에이전트 B<br/>자체 CLAUDE.md<br/>자체 skills<br/>자체 KB<br/>자체 MCP 도구"]
        SC["서브에이전트 C<br/>자체 CLAUDE.md<br/>자체 skills<br/>자체 KB<br/>자체 MCP 도구"]
    end
    ...
    subgraph OrcBot["오케스트레이터 · 어셈블리"]
        direction TB
        H["4 · 핸드오프<br/>summary + key_findings만 읽음<br/>전체 result.md는 파일 경로로만 참조"]
