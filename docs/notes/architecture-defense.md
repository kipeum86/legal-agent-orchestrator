...
3. 웹 서버 유지보수 불필요
4. 포트폴리오 프로젝트로 완벽 (실행 가능한 아티팩트 + 정적 리플레이)
...
>
> We inverted the tradeoff: **use Claude Code as the runtime, preserve 100% of agent capability, decouple visualization into static Case Replay.** The result is an architecture that runs real legal work, not a demo.
...
>
> Case Replay turns every processed case into a shareable, replayable artifact. Static JSON + static viewer = no API key required, works offline, embeddable in a tweet.
>
> The legal process is the content. The visualization is how we deliver it.
