#!/bin/bash
# setup.sh — 9명의 하위 에이전트를 항상 최신 main으로 동기화 (shallow clone)
...
# Legal Agent Orchestrator가 호출하는 9명의 하위 에이전트.
...
case "${1:-setup}" in
  setup|update)
    ...
    echo "✅ All 9 subordinate agents are at latest $DEFAULT_BRANCH."
    ;;
  status)
