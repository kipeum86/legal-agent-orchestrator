...
| "양측 의견을 들려줘" / "논쟁 보고 싶다" | `multi` | (상황) | `debate` | `adversarial` | Pattern 3 → `manage-debate.md` *(현재 skeleton, Pattern 1으로 fallback)* |
...
질문 입력
  ...
  │   → skills/manage-debate.md 참조 (현재 skeleton)
  │   → fallback: 같은 에이전트 조합으로 Pattern 1 병렬 실행
  │
...
## Step 6: Pattern 3 참조 (토론)

`complexity == "adversarial"`일 때 `skills/manage-debate.md` 참조. 현재 **skeleton 상태** — 실제 토론 로직은 Phase 2 후속 작업.

### Fallback 동작 (토론 미구현 상태)

사용자가 명시적으로 토론/논쟁을 요청했으므로, 단순 Pattern 1 병렬로 대체하면 **의도가 손실됨**. 다음 절차로 의도를 최대한 보존:

1. **이벤트 로깅**: `pattern_3_deferred`로 토론 의도가 감지되었으나 Pattern 1 fallback됨을 기록.
   ```bash
   echo '{"id":"evt_NNN","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","agent":"orchestrator","type":"pattern_3_deferred","data":{"user_intent":"debate","fallback":"pattern_1_with_conflict_surfacing","reason":"manage-debate.md is skeleton"}}' >> "$PROJECT_ROOT/output/$CASE_ID/events.jsonl"
   ```

2. **사용자 안내**: "멀티라운드 토론은 아직 구현 중입니다. 현재는 각 전문가의 독립 분석(Pattern 1)으로 처리하되, **상충점을 명시적으로 식별**하여 의견서에 포함합니다."

3. **Pattern 1 실행** (Step 5 절차 준수) — 단, writing-agent 프롬프트에 다음 **Pattern 3 의도 보존 지시** 주입:
   ```
   [중요: 사용자는 토론/양측 의견을 명시적으로 요청했습니다]

   단순히 각 전문가의 독립 분석을 나열하지 마세요. 다음을 추가로 수행:

   1. **명시적 충돌점 식별**: 두 관할권/전문가의 법적 입장이 서로 다른 지점을 명시적으로 표로 정리.
      | 쟁점 | {관할권 A} 입장 | {관할권 B} 입장 | 충돌 여부 |

   2. **반대 논거 서술**: 각 관할권이 반대 입장을 **어떻게 주장할 수 있는지** 서술 (가상 반박).

   3. **"진정한 토론이었다면" 섹션**: 이 질문을 토론 패턴으로 처리했다면 논의되었을 핵심 쟁점 2-3개를 bullet로 제시.

   이렇게 하여 사용자의 토론 의도를 최대한 보존합니다.
   ```

4. writing-meta.json에 `pattern_3_fallback: true` 플래그 기록.
...
<!-- IF pattern_3_fallback == true (토론 요청 → Pattern 1 대체) -->
[Pattern 3 의도 보존 지시 — Step 6 참조]
사용자가 토론/양측 의견을 명시적으로 요청했습니다. 단순 나열이 아닌 다음을 수행:
1. 명시적 충돌점 표 (| 쟁점 | {A} 입장 | {B} 입장 | 충돌 여부 |)
2. 각 관할권의 반대 논거 서술
3. "진정한 토론이었다면" 섹션 (핵심 쟁점 2-3개 bullet)

writing-meta.json에 pattern_3_fallback: true 플래그 추가.
<!-- END IF -->
...
# meta.json 추가 필드: pattern (pattern_1|pattern_2|pattern_3_fallback), partial_results (bool)
```
...
- partial_results 또는 pattern_3_fallback 플래그가 있으면 해당 맥락을 검토에 반영
...
v2에서 추가/확장된 이벤트 타입. Case Replay (Phase 3) 및 디버깅을 위해 중앙 참조.
...
| `pattern_3_deferred` | **v2** | `user_intent`, `fallback`, `reason` | 토론 요청이 Pattern 1로 대체됨 |
...
- 향후 Case Replay 뷰어가 파싱하므로 스키마 변경 시 본 부록 업데이트 필수.
...
| Pattern 3 fallback (2-agent) | Pattern 1 + Pattern 3 의도 보존 추가 작업 | ~170k | ~280k | **~450k** | 출력 증가 |
